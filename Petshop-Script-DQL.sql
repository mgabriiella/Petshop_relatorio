...
SELECT 
    ...
    t.numero AS "telefone"     -- corrigido para 'numero'
FROM 
    empregado e   
JOIN 
    telefone t ON e.cpf = t.empregado_cpf   -- a chave de junção correta é 'empregado_cpf'
WHERE 
...
ORDER BY 
    v.data DESC;  -- ordena pela data do serviço da mais recente para a mais antiga
...
ORDER BY 
    v.data DESC;  -- ordena pela data da venda da mais recente para a mais antiga
...
SELECT 
    f.tipo AS "tipo forma pagamento",                -- tipo da forma de pagamento
    COUNT(v.idvenda) AS "quantidade vendas",         -- contagem de vendas para cada forma de pagamento
    SUM(f.valorPago) AS "total valor vendido"        -- soma total dos valores pagos
FROM 
    petshop.formapgvenda f                           -- tabela de formas de pagamento
JOIN 
    petshop.venda v ON f.venda_idvenda = v.idvenda  -- relaciona formas de pagamento com vendas
GROUP BY 
    f.tipo                                           -- agrupa por tipo da forma de pagamento
ORDER BY 
    COUNT(v.idvenda) DESC                            -- ordena pela quantidade total de vendas
LIMIT 10;                                          -- limita a 10 resultados

-- Relatório 10: balanço das vendas
SELECT 
    DATE(c.datacomp) AS "data compra",                  -- data da compra
    COUNT(c.idcompra) AS "quantidade de compras",       -- contagem de compras por dia
    SUM(c.valortotal) AS "valor total compra"            -- soma total dos valores das compras
FROM 
    petshop.compras c                                    -- tabela de compras
GROUP BY 
    DATE(c.datacomp)                                     -- agrupa os resultados por data da compra
ORDER BY 
